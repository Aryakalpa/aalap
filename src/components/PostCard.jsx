...
export default function PostCard({ post, onUpdate }) {
  ...
  return (
    <Link to={`/post/${post.id}`} className="card fade-in" style={{ display: 'block' }}>
      <div className="post-row">
        <div className="post-row-main">
                ...
                {showMenu && (
                  <div className="share-menu fade-in" style={{ right: 0, top: '100%' }}>
                    <div className="share-menu-item" onClick={(e) => {
                      e.preventDefault(); e.stopPropagation(); window.location.href = `/write/${post.id}`
                    }}>
          ...
          <p className={['poem', 'poetry'].includes(post.category?.toLowerCase()) ? 'post-excerpt poem-preview' : 'post-excerpt'}>
            {generateExcerpt(post.body || '', 220)}
          </p>

          <div className="post-actions">
            <div className="inline-stats">
              <button className={`btn-ghost ${liked ? 'text-danger' : ''}`} onClick={handleLike}>
            ...
            <div className="inline-actions">
              <button className={`btn-ghost ${bookmarked ? 'text-accent-blue' : ''}`} onClick={handleBookmark} title="Save for later">
