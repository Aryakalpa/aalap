...
export default function Feed({ type }) {
  ...
  const fetchPosts = async () => {
    setLoading(true);
    // Minimal artificial delay to show off the beautiful skeleton (optional)
  ...
  return (
    <div onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ paddingBottom: 100, minHeight: '80vh' }}>
      ...
      {loading && pullY === 0 ? (
          ...
          posts.length === 0 ? <div style={{ padding: 40, textAlign: 'center', opacity: 0.5 }}>কোনো কাহিনী পোৱা নগ’ল</div> : posts.map(post => <PostCard key={post.id} post={post} />)
      )}
