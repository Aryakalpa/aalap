...
export default function Reader() {
  ...
  const { user, toggleBookmark, bookmarks, setTab, setShareTarget } = useStore();

  // Use passed state (if clicked from Feed) or null (if direct link)
  ...
  const isSaved = post && Array.isArray(bookmarks) && bookmarks.some(b => b && b.id === post.id);

  // Fetch Post if not available
  useEffect(() => {
    if (!post && id) {
      setLoading(true);
      supabase.from('posts').select(`*, profiles(*)`).eq('id', id).single().then(({ data, error }) => {
        if (data) setPost(data);
        if (error) toast.error('Error loading post');
        setLoading(false);
      });
    }
  }, [id, post]);
  ...
  const authGuard = () => { if (!user) { toast.error('অনুগ্ৰহ কৰি লগ-ইন কৰক'); navigate('/profile'); return false; } return true; };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-body)', position: 'relative' }}>
      <div style={{ position: 'sticky', top: 0, background: 'var(--bg-body)', zIndex: 10, paddingBottom: 20, display: 'flex', alignItems: 'center', gap: 15, borderBottom: '1px solid var(--border-light)' }}>
        <button onClick={() => navigate(-1)} className="btn-icon" style={{ padding: 10 }}><ArrowLeft /></button>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {author.avatar_url && <Avatar url={author.avatar_url} size={32} />}
          <span style={{ fontWeight: 700, fontSize: 14 }}>{author.display_name || 'Anonymous'}</span>
        </div>
      </div>
      ...
      <div className="reader-bar">
        ...
        <button onClick={() => navigate(`/post/${post.id}/comments`)} className="btn-icon" style={{ color: 'var(--text-main)' }}><MessageCircle size={24} /></button>
