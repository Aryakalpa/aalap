...
export default function EchoChamber() {
  ...
  useEffect(() => {
    if (!id) return;
    supabase.from('comments').select(`*, profiles(*)`).eq('post_id', id).order('created_at', { ascending: false })
  ...
  const send = async () => {
    ...
    if (!text.trim()) return;
    const { error } = await supabase.from('comments').insert({ post_id: id, user_id: user.id, body: text });
    if (!error) {
      setText('');
      supabase.from('comments').select(`*, profiles(*)`).eq('post_id', id).order('created_at', { ascending: false })
        .then(({ data }) => setComments(data || []));
