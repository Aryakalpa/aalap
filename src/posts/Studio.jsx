...
export default function Studio() {
  ...
  const isEditing = !!id;
  const passedPost = location.state;
  const [editPost, setEditPost] = useState(passedPost || {});

  // Fetch post if editing and no passed state
  useEffect(() => {
    if (isEditing && !passedPost && !editPost.id) {
  ...
  }, [id, passedPost, isEditing, editPost.id]);
  ...
  const [category, setCategory] = useState(() => isEditing ? (editPost.category || 'story') : 'story');
  const [cover, setCover] = useState(() => isEditing ? (editPost.cover_image || null) : null);
  ...
  if (!user) {
    // Redirect handled by effect or component logic. 
    // Avoiding duplicate navigation here, just return null and effect will catch, or button calls.
    // But simpler:
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        <p>অনুগ্ৰহ কৰি লগ-ইন কৰক</p>
        <button onClick={() => navigate('/profile')} className="btn-soft">লগ-ইন</button>
      </div>
    );
  }
